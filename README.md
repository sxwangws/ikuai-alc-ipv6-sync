...
git clone https://github.com/sxwangws/ikuai-acl-ipv6-sync.git
...
docker run -d \
  ...
  -v $(pwd)/data:/app/data \
  sxwangws/ikuai-acl-ipv6-sync:latest
...
• 跳过提示
🛠 How to quickly deploy Docker / 快速 Docker 部署（飞牛OS 示例）

1. 在部署的 Docker 文件夹下新建目录，如 ikuai-acl-ipv6-sync；

2. 在此目录下再创建 data 目录，将项目中的 config.example.yml 下载到该目录下；

3. 把 config.example.yml 改名为 config.yml，同时修改配置文件中的信息并保存；

4. 打开飞牛 Docker，选择 Compose，点击新增项目，项目名称自取，路径选择上面新建的目录（如 ikuai-acl-ipv6-sync），来源选择“创建 docker-compose.yml”，将以下代码复制进去：
version: '3.8'
...
6. 注意：相关需要添加的设备需要开启 SSH，目前支持 OpenWrt 和飞牛，其他设备暂未适配。
⭐ Support
